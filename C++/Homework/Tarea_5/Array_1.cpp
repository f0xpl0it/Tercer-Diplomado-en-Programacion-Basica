//============================================================================
// Name        : Array_1.cpp
// Author      : Michael Paucar
// Version     : v.1
// Copyright   : Copyright (c) 2025 Michael Paucar.
// Description : Tarea 5 - Arreglos (Ejercicio [1])
//============================================================================

#include <iostream>
#include <limits>
using namespace std;

// EJERCICIO 1

// Funci¢n para limpiar errores de entrada
void limpiarEntrada() {
  cin.clear();
  cin.ignore(numeric_limits<streamsize>::max(), '\n');
}

// Funci¢n para verificar si la entrada fue v lida
bool entradaValida() {
  if (cin.fail()) {
    cout << "\033[31m\nSolo se permiten n£meros!!!\n\033[0m";
    limpiarEntrada();
    return false;
  }
  limpiarEntrada();  // Siempre limpiar el buffer
  return true;
}

// Funci¢n para encontrar n£mero mayor o menor
void arreglo() {
  int n;
  cout << "\nIngrese la longitud del arreglo: ";
  cin >> n;

  while (!entradaValida() || n <= 0) {
    cout << "\nReingrese la longitud del arreglo: ";
    cin >> n;
  }

  int numeros[n];  // Crear el arreglo

  for (int i = 0; i < n; i++) {
    cout << "Ingrese el valor para la posici¢n [" << i + 1 << "]: ";
    cin >> numeros[i];

    while (!entradaValida()) {
      cout << "Reingrese el valor para la posici¢n [" << i + 1 << "]: ";
      cin >> numeros[i];
    }
  }

  int mayor = numeros[0];
  int menor = numeros[0];

  cout << "\nARREGLO -> [ ";
  for (int i = 0; i < n; i++) {
    cout << numeros[i] << " ";

    if (numeros[i] > mayor) mayor = numeros[i];
    if (numeros[i] < menor) menor = numeros[i];
  }
  cout << "]\n";

  cout << "\n>> N£mero mayor: " << mayor << endl;
  cout << ">> Numero menor: " << menor << endl;
}

// Funci¢n Principal
int main() {
  cout << "\n======== EJERCICIO 1 - NéMERO MAYOR Y MENOR ========\n";

  arreglo();

  return 0;
}
